...
#  Maps provider names to their API endpoint patterns and model name strings.
#  These are the "fingerprints" we search for in the DEX string pool.
# ─────────────────────────────────────────────────────────────
...
GENAI_PROVIDER_MAP: dict[str, dict] = {
    ...
    "Ollama": {
        ...
        "key_pattern": None,   # Ollama has no API key
...
# LLM message JSON schema patterns (indicates structured LLM prompting code)
...
class GenAIScaffoldDetector:
    ...
    def __init__(self) -> None:
        # Pre-flatten all endpoint and model strings for O(1) lookup construction
        self._all_endpoints: dict[str, str] = {}   # endpoint_token → provider_name
        self._all_models:    dict[str, str] = {}   # model_name → provider_name
    ...
    def run(
        ...
        # Build a set of dead method descriptors for O(1) lookup
        ...
        # Pass 1: Scan DEX string pool for GenAI indicators
        ...
        # Pass 2: Scan dead code Smali directly (for obfuscated endpoint references)
    ...
    def _scan_string_pool(
        ...
        try:
            for string_analysis in analysis.get_strings():
                ...
                # Find methods that contain this string constant
                for method_analysis, _ in string_analysis.get_xref_from():
                    ...
                    # Only flag if this method is dead (unreachable)
                    if desc not in dead_descriptors and not self._is_stub_method(method_analysis):
                        continue

                    # Extract additional intelligence from the method body
    ...
    def _scan_dead_code_smali(
        ...
        for dead in dead_code:
            ...
            # Extract all string constants from this method's Smali
            ...
            for match in string_re.finditer(smali):
                ...
                # Also check for LLM message schema patterns in the same method
                ...
                break   # One artifact per dead method (avoid duplicates from same method)
    ...
    def _classify_string(self, value_lower: str, raw_value: str) -> tuple[str, str]:
        ...
        """
        # Check endpoint patterns (most specific)
        ...
        for provider, data in GENAI_PROVIDER_MAP.items():
            ...
            if key_pattern and key_pattern.search(raw_value):
                if len(raw_value) > 20:   # Avoid false positives from short matches
