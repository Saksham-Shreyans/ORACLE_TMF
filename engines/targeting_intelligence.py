...
# Mobile Country Codes (MCC) → country names + ISO codes
# Source: ITU-T E.212 MCC assignments
...
# Locale directory suffix → country ISO mapping
...
# HTML overlay patterns indicating banking phishing content
...
# Package name patterns for known malware families
...
class TargetingIntelligence:
    ...
    def __init__(self) -> None:
        ...
        # Build lookup structures for O(1) package matching
        self._pkg_to_entry: dict[str, dict] = {
            entry["package_name"]: entry
            for entry in self._taxonomy.get("entries", [])
        }
        # Also index by institution name fragments for substring matching
    ...
    def run(
        ...
        targets:       dict[str, dict] = {}   # package_name → target entry
        ...
        # ── Compute final confidence per target ───────────────────────
        predicted = []
        for pkg, tgt in targets.items():
            layers = tgt.get("detection_layers", [])
            # Confidence increases with each independent layer that confirms the target
        ...
        # Overall targeting confidence = average of top-3 targets
        ...
        return {
            "predicted_targets":      predicted[:10],   # Top 10 most confident targets
    ...
    def _layer1_package_arrays(self, mag: MutationArtifactGraph) -> list[dict]:
        ...
        for string_val in dead_strings:
            ...
            # Package prefix match (e.g., "com.hdfc" matches multiple HDFC apps)
    ...
    def _layer2_overlay_assets(
        ...
        for ui_flow in mag.unfinished_ui_flows:
            # Check asset reference names against institution keywords
            for asset_ref in ui_flow.asset_refs:
                asset_lower = asset_ref.lower()
                for institution_kw, entry in self._institution_keywords:
                    # Check if the institution name fragment appears in the asset name
            ...
            # Also check layout file name itself
    ...
    def _layer3_geographic_signals(
        ...
        # a) Locale directories in extracted APK
            ...
            if os.path.isdir(values_dir):
                for subdir in os.listdir(values_dir):
                    if subdir.startswith("values-"):
                        locale_suffix = subdir[7:]   # Strip "values-"
                        # Normalize: "values-hi-rIN" → "hi"
        ...
        # b) MCC codes in dead code string constants
        ...
        # c) Country ISO codes in placeholder strings
    ...
    def _layer4_html_overlays(
        ...
        # Scan placeholder strings (high-entropy or marker strings from Stage F)
        ...
        # Also scan C2 stub payload schemas
        for stub in mag.c2_stubs:
        ...
        for html_chunk in html_candidates:
            # Check against HTML bank indicator patterns
            for pattern in _HTML_BANK_INDICATORS:
                ...
                if m:
                    ...
                    # Try to identify institution from matched text
                    for institution_kw, entry in self._institution_keywords:
                        inst_tokens = institution_kw.split()
    ...
    def _load_taxonomy() -> dict:
        ...
        # Inline fallback with critical Indian + global banks
