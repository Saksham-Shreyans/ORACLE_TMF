"""
ORACLE-TMF  ·  models/nav_models.py
=====================================
Data models for Negative Artifact Vectors (NAV) — Stage 2 Tier 2.

NAV tracks artifact DISAPPEARANCE across versions, complementing the MVV
which tracks artifact accumulation.  A sudden drop from positive to zero
in Scaffolding artifacts signals the developer abandoned that path and
shifted to an alternative — redirecting the forecast accordingly.

NAV-MIRAGE detector: artifacts that rapidly appear across 1–2 versions
then completely vanish are flagged as potential adversarial poisoning
attempts (injected to manipulate the forecast).

These models are free of external dependencies so any stage can import
...
class NAVEventType(str, Enum):
    """Classification of a Negative Artifact Vector event."""
...
class NAVRedirectionHypothesis(str, Enum):
    """Forecast redirection implied by a NAV event."""
...
class NAVEvent:
    """
    A single Negative Artifact Vector event — one class of artifacts
    transitioning from present to absent between two consecutive versions.

    Fields
    ------
    artifact_class : str
        ArtifactClass.value of the disappearing artifact class.
    version_from : str
        Version string where artifact was last present.
    version_to : str
        Version string where artifact was first absent.
    count_before : int
        Artifact count in the earlier version.
    count_after : int
        Artifact count in the later version (typically 0).
    delta_count : int
        count_before - count_after (always positive for a NAV event).
    event_type : NAVEventType
        Classification of why the artifact disappeared.
    redirection_hypothesis : NAVRedirectionHypothesis
        Forecast redirection implied by this disappearance.
    nav_score : float
        Normalised NAV score [0.0, 1.0] for Stage K weighting.
    mirage_confidence : float
        Confidence this is an adversarial poisoning attempt [0.0, 1.0].
    supporting_evidence : list[str]
        Human-readable evidence strings for the report.
    """
...
class NAVHistory:
    """
    Multi-version NAV history for a single malware family — used by the
    NAV-MIRAGE detector to identify the rapid appearance-then-disappearance
    pattern across at least NAV_MIRAGE_MIN_VERSIONS versions.

    Built incrementally as each version is analysed.
    Stored in the per-family knowledge base for longitudinal analysis.
    """
...
class NAVResult:
    """
    Output of the NAVEngine.run() method.
    Aggregates all NAV events detected between two consecutive versions.
    """
    ...
    def to_dict(self) -> dict:
        """Serialise to dict for JSON output and Stage K integration."""
        return {
            "nav_events": [
                {
