...
class ORACLETMFOrchestrator:
    ...
    def analyze(
        ...
        try:
            ...
            # ── TMF-REFLECT: Augment CFG with reflection edges ─────────
            if analysis and cfg:
            ...
            # Detect malware family from manifest package name if not yet set
            ...
            if analysis and cfg:
                ...
                # ── DTE: Dormancy Taxonomy Classification ─────────────
            ...
            # ── STAGE E: Unused Permission Analysis ────────────────────
            ...
            # ── STAGE F: String Resource Mining ───────────────────────
            if analysis:
            ...
            # ── CLASS 7 — GenAI Scaffold Detection (TMF-Psi) ──────────
            if analysis:
            ...
            # Store targeting data for Stage J context
            mag.manifest["_targeting"] = targeting

            # ── STAGE I: Version Diff (optional — requires prev APK) ───
            ...
            # ── STAGE J: LLM Multi-Agent Reasoning ────────────────────
            if not skip_llm:
                ...
                # ── STAGE K: Bayesian Confidence Scoring ────────────
                ...
                # Propagate targeting intelligence to forecasts
                self._apply_targeting_to_forecasts(mag, targeting)

            # ── STAGE L: Report Synthesis ──────────────────────────────
            report_bundle = None
            if not skip_report:
    ...
    # ─────────────────────────────────────────────────────────
    #  STATIC-ONLY ANALYSIS (for previous version comparison)
    # ─────────────────────────────────────────────────────────
