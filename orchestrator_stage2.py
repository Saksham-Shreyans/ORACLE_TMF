"""
ORACLE-TMF  ·  orchestrator_stage2.py
=======================================
Stage 2 Orchestrator — integrates all Tier 2 and Tier 3 engines
around the existing Stage 1 pipeline output (Stages A–L).
...
Execution order (all optional, controlled by Stage2Config):
  Stage M  — PHANTOM detonation (requires phantom_enabled=True)
  Stage N  — NAV analysis (always runs if mag_prev is provided)
  Stage O  — CABAL collusion (requires ≥2 APKs in mag_list)
  Stage P  — KINSHIP fingerprinting
  Stage Q  — MIRAGE robustness analysis
  Stage R  — OUROBOROS co-evolution (requires ouroboros_enabled=True)
  Network  — DDoS/ANC signature detection
  Synth    — Synthetic variant generation (research mode)
...
class Stage2Config:
    ...
    # Core Stage 2 (Tier 2)
    phantom_enabled: bool = False       # Requires controlled lab environment
    nav_enabled: bool = True            # Always recommended when prev version exists
    use_llm_for_cabal: bool = True      # LLM-enhanced collusion scoring

    # Research Mode (Tier 3)
    cabal_enabled: bool = False         # Multi-APK collusion (set when analysing app sets)
    kinship_enabled: bool = True        # Builder attribution (recommended)
    mirage_enabled: bool = True         # Robustness analysis (recommended)
    ouroboros_enabled: bool = False     # Co-evolution (training mode only)
    network_attack_enabled: bool = True # DDoS signature detection (always recommended)
    synthetic_variant_enabled: bool = False  # Test fixture generation (research only)
...
class Stage2Report:
    ...
    # Aggregated enrichments for Stage L PDF
    ...
    # Adjusted forecasts (after NAV confidence adjustment)
    ...
    def to_dict(self) -> dict:
        """Serialise to dict for JSON output and Stage L PDF integration."""
        return {
            ...
            "stage_results": {
                ...
                "NETWORK_ATTACK": self.network_attack.to_dict() if self.network_attack else None,
            },
            "adjusted_forecasts": [
                {
...
class Stage2Orchestrator:
    ...
    Each engine failure is isolated — the report is always returned.
    ...
    def __init__(self, config: Optional[Stage2Config] = None) -> None:
        ...
        # Initialise all stages (only enabled ones are actually used)
        ...
        # Persistent NAV history (loaded from disk on init, saved after each run)
    ...
    def run(
        ...
        ----------
        mag : MutationArtifactGraph
            Primary APK MAG (Stages A–L already complete).
        ...
        working_forecasts = list(forecasts)  # Copy to avoid mutating Stage K output
        ...
        if self.config.phantom_enabled:
            ...
            try:
                ...
                if report.stage_m and not report.stage_m.skipped:
                    ...
                    # Apply confidence boosts from confirmed behaviors
        ...
        # ── Finalise ─────────────────────────────────────────────────────────
        # Re-evaluate gate on adjusted forecasts
        for f in working_forecasts:
