"""
ORACLE-TMF  ·  phantom/deception_engine.py
============================================
PHANTOM Active Deception Engine — Stage 2 Tier 2.

PHANTOM coerces dormant payloads into full execution within a synthetic
environment by simulating a complete Android device using LLM-powered
stateful responses.  It resolves the Honeypot Trilemma:
  • Flexibility — LLM adapts to any C2 command or API query
  • Interaction depth — multi-turn stateful session context
  • Fidelity — OU sensor data, Luhn-valid credentials, real OTP formats

Architecture
------------
  PhantomSession maintains state across the detonation:
    ├── DevicePersonaGenerator   → Android build fingerprint
    ├── HoneytokenGenerator      → Synthetic financial artifacts
    ├── SensoryEmulator          → OU sensor streams
    ├── BehavioralBiometricGen   → Keystroke / touch timing
    └── LLM (Claude)             → Stateful command simulator

Each turn in the detonation is a call to PhantomDeceptionEngine.respond():
  1. The malware C2 sends a command (JSON/HTTP payload).
  2. PHANTOM builds an LLM prompt with the full device context.
  3. Claude simulates the Android response.
  4. The response is returned to the malware.
  5. All C2 traffic is captured to a PCAP-equivalent log.

Safety Boundary
---------------
  • All financial data = per-session honeytokens (no real credentials).
  • Sensor streams = synthetic OU (no real device data).
  • PCAP retained only for PHANTOM_PCAP_RETAIN_HOURS hours.
  • PHANTOM never sends real network traffic (localhost proxy only).
"""
...
@dataclass
class PhantomTurn:
    """A single turn in a PHANTOM detonation session."""

    turn_index: int = 0
    timestamp: float = 0.0
    malware_command: str = ""           # Raw C2 payload / API call received
    phantom_response: str = ""          # LLM-generated response
    sensor_snapshot: dict = field(default_factory=dict)  # Sensor state at this turn
    detected_behaviors: list[str] = field(default_factory=list)  # Behaviors observed
    exfiltration_attempts: list[str] = field(default_factory=list)  # Honeytoken leaks


@dataclass
class PhantomSession:
    """
    Full state for a PHANTOM detonation session.

    Persisted across turns and serialised for the PCAP-equivalent log.
    """
    ...
    def to_dict(self) -> dict:
        """Serialise for the PHANTOM detonation log."""
        return {
...
class PhantomDeceptionEngine:
    ...
    Orchestrates a full LLM-powered stateful Android simulation for
    coercing dormant malware payloads into execution.

    Usage
    -----
    >>> engine = PhantomDeceptionEngine()
    >>> session = engine.start_session(apk_path="/tmp/malware.apk")
    >>> response = engine.respond(session, command='{"action": "get_sms"}')
    >>> log = engine.end_session(session)
    """
    ...
    def start_session(
        ...
        """
        Start a new PHANTOM detonation session.

        Parameters
        ----------
        apk_path : str
            Path to the malware APK under analysis.
        persona_index : int | None
            Specific device persona to use.  None = random.
    ...
    def respond(
        ...
        """
        Process one malware command and return the simulated Android response.

        Parameters
        ----------
        session : PhantomSession
            Active detonation session (mutated in-place).
        command : str
            Raw C2 command or API call from the malware.

        Returns
        -------
        str
            Simulated Android/server response.
        """
        ...
        # Detect honeytoken exfiltration attempts
        exfil_attempts = self._detect_exfiltration(command, session.bundle)

        # Build LLM prompt for stateful simulation
        ...
        # Accumulate captured behaviors
        session.behaviors_captured.extend(turn.detected_behaviors)

        # If OTP was exfiltrated, refresh it (persistent adversary simulation)
    ...
    def end_session(
        ...
        """
        Terminate a PHANTOM session and return the completed log.

        Parameters
        ----------
        session : PhantomSession
            The active session to terminate.
        reason : str
            Termination reason: "normal", "timeout", "turn_limit", "error".

        Returns
        -------
        PhantomSession
            Completed and sealed session with full detonation log.
        """
    ...
    def _build_simulation_prompt(
        ...
        """
        Build the LLM system + user prompt for one simulation turn.

        The system prompt establishes the deception environment.
        The user message contains the malware command to simulate responding to.
        """
        ...
        # Build conversation history (last N turns for context)
        history_ctx = ""
        recent = session.turns[-5:]  # Keep last 5 turns to avoid context bloat
        for turn in recent:
            history_ctx += (
    ...
    def _call_llm(self, prompt: str, session: PhantomSession) -> str:
        """
        Call the Anthropic API for stateful simulation.

        Falls back to a deterministic stub response if the API is
        unavailable (for testing without API credentials).
        """
        ...
        try:
            ...
            msg = client.messages.create(
                model=LLM_MODEL,
                max_tokens=512,  # Keep responses concise (real Android responses are short)
    ...
    def _stub_response(prompt: str) -> str:
        """Deterministic stub for testing without LLM API access."""
    ...
    def _detect_exfiltration(
        ...
        """
        Detect if the malware command contains exfiltrated honeytoken data.
        """
        if bundle is None:
            return []
    ...
    def _classify_behaviors(command: str, response: str) -> list[str]:
        """
        Classify malware behaviors observed in this turn.
        Returns a list of behavior labels.
        """
        behaviors: list[str] = []
        cmd_lower = command.lower()
