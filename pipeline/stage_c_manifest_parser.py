...
class ManifestParser:
    ...
    # Meta-data keys that hint at GenAI API integration (TMF-Psi detection)
    ...
    def _extract_meta_data(self, apk: Any) -> dict:
        ...
        meta: dict = {}
        try:
            # Androguard exposes get_declared_permissions_details() but not meta-data
            # directly.  We parse the manifest XML string for <meta-data> tags.
    ...
    def _is_exported(apk: Any, component_name: str, comp_type: str) -> bool:
        ...
        """
        try:
            # Androguard exposes this via the manifest XML
    ...
    def _get_intent_filters(apk: Any, component_name: str, comp_type: str) -> list[dict]:
        ...
        try:
            ...
            # Find the component block and scan for <intent-filter> inside it
            short_name = component_name.split(".")[-1]
            block_re = re.compile(
                rf'<{comp_type}[^>]*(?:{re.escape(component_name)}|{re.escape(short_name)})'
                rf'.*?</{comp_type}>',
