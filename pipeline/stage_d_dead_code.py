...
class DeadCodeDetector:
    ...
    def __init__(self) -> None:
        # Precompile lifecycle prefix check
    ...
    def run(
        ...
        # 1. Collect all entry points (Android lifecycle roots for BFS)
        ...
        try:
            for class_analysis in analysis.get_classes():
                ...
                for method_analysis in class_analysis.get_methods():
                    ...
                    # Always include lifecycle callbacks regardless of component class
                    if self._lifecycle_re.match(method_name):
                        desc = self._method_desc(method_obj)
                        entry_descriptors.add(desc)

                    # Include all methods of declared component classes
                    if is_component:
                        desc = self._method_desc(method_obj)
                        entry_descriptors.add(desc)
    ...
    def _inject_reflection_edges(self, analysis: Any, cfg: Any) -> None:
        ...
        try:
            for method_analysis in analysis.get_methods():
                ...
                # Extract class names from const-string opcodes in this method
                string_literals = re.findall(r'const-string[^,]+,\s*"([^"]+)"', smali_code)
                for literal in string_literals:
                    # Convert dot-notation to Smali class descriptor
                    smali_candidate = "L" + literal.replace(".", "/") + ";"
                    # Check if this class exists in the analysis
                    try:
                        for class_analysis in analysis.get_classes():
                            if class_analysis.name == smali_candidate:
                                # Add synthetic edge to all methods in that class
    ...
    def _bfs_reachable(self, cfg: Any, entry_points: set[str]) -> set[str]:
        ...
        if cfg is None:
            ...
            return set()   # Will cause all methods to appear "reachable" — safe fallback
        ...
        while queue:
            ...
            for _, callee, _ in cfg.out_edges(node, data=True) if False else []:
                pass
            # Use NetworkX successors for directed graph
            try:
    ...
    def _collect_dead_methods(
        ...
        try:
            for method_analysis in analysis.get_methods():
                ...
                # Filter 1: Skip external/Android framework methods
                ...
                # Filter 2: Skip lifecycle callbacks and constructors
                if self._lifecycle_re.match(method_name):
                    continue

                # Filter 3: Skip constructors and static initialisers
                if method_name in ("<init>", "<clinit>"):
                    continue

                desc = self._method_desc(method_obj)

                # Filter 4: Method must not be reachable
                if desc in reachable:
                    continue

                # Filter 5: Must have callers check via Androguard xref
                xrefs_from = list(method_analysis.get_xref_from())
                if xrefs_from:
                    # Has callers in xref — not truly dead (CFG might miss it)
                    continue

                # Filter 6: Minimum opcode threshold (skip trivial getters/setters)
                smali_code = self._get_smali(method_analysis)
                opcode_count = self._count_opcodes(smali_code)
                if opcode_count < DEAD_CODE_MIN_OPCODE_COUNT:
                    continue

                # Extract DTE feature vector
                ...
                artifact = DeadCodeArtifact(
                    ...
                    smali_code     = smali_code[:4000],  # Cap at 4 KB per method
    ...
    def _compute_guard_entropy(smali_code: str) -> float:
        ...
        if not smali_code:
            return 0.0
        import math
        # Extract operands of if-* instructions
