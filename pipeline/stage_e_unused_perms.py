...
# Maps permission names to human-readable permission groups
...
class UnusedPermissionAnalyzer:
    ...
    def run(self, manifest: dict, analysis: Any) -> list[UnusedPermissionArtifact]:
        ...
        # Build the corpus of all invoked API signatures in the APK
        ...
        # Cross-reference each permission
    ...
    def _build_invoked_signature_corpus(self, analysis: Any) -> set[str]:
        ...
        try:
            for method_analysis in analysis.get_methods():
                # Only care about calls TO external / framework methods
                for _, callee_method, _ in method_analysis.get_xref_to():
                    if callee_method is None:
                        continue

                    # Build the callee descriptor
                    try:
    ...
    def _check_permission(
        ...
        """
        # Look up the Axplorer-inspired API mapping
        expected_apis = PERMISSION_TO_API_MAP.get(permission)
        if not expected_apis:
            # Permission not in our mapping — skip (insufficient data to judge)
            return None

        # Check if ANY expected API appears in the invoked corpus
        for api_pattern in expected_apis:
            for sig in invoked_signatures:
                if api_pattern in sig:
                    # Found a match → permission IS being used
                    return None

        # No match found → this permission is declared but never used
