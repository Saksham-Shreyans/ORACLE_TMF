...
# Pre-compile all placeholder patterns
_COMPILED_PATTERNS: dict[str, re.Pattern] = {
    name: re.compile(pattern, re.IGNORECASE)
    for name, pattern in PLACEHOLDER_PATTERNS.items()
}

# Known benign high-entropy strings (base64 prefixes, common SDK tokens)
# to avoid false positives
_BENIGN_PREFIXES: tuple[str, ...] = (
    "AAAAgASU",      # Google FCM token prefix
    "com.google.",
    "com.android.",
    "android.util.",
    "java.lang.",
...
class StringMiner:
    ...
    def _mine_dex_string_pool(self, analysis: Any) -> list[PlaceholderStringArtifact]:
        ...
        try:
            # Androguard exposes string_analysis objects via get_strings()
            for string_analysis in analysis.get_strings():
                value = str(string_analysis.get_value())

                # Skip short or benign strings
