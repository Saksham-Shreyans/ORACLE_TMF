...
# Patterns for HTTP method annotation strings in Smali (Retrofit style)
_HTTP_METHOD_RE = re.compile(
    r'const-string[^,]+,\s*"((?:GET|POST|PUT|DELETE|PATCH|HEAD)\b[^"]*)"',
    re.IGNORECASE,
)

# URL/path extraction from const-string values
_URL_RE = re.compile(
...
class C2StubDetector:
    ...
    def __init__(self) -> None:
        # Precompile network client patterns for faster matching
    ...
    def run(
        ...
        # Pass 1: Scan dead-code methods for network scaffolding
        for dead in dead_methods:
            artifact = self._check_dead_method(dead)
            if artifact is not None:
                stubs.append(artifact)

        # Pass 2: Scan ALL methods (including reachable) for C2 stubs
        # Some stubs exist in reachable code but are gated by flags → still scaffolds
    ...
    def _check_dead_method(
        ...
        # Check for network client class references
        framework = self._detect_network_client(smali)
        if not framework:
            return None

        # Check for terminal execution methods
        if self._has_terminal_method(smali):
            # Terminal method present → this network code actually executes
            return None

        # Extract URL/path and HTTP method
    ...
    def _scan_all_methods(
        ...
        try:
            for method_analysis in analysis.get_methods():
                ...
                # Only emit if there's an extractable URL or HTTP method hint
    ...
    def _extract_payload_schema(smali: str) -> str:
        ...
        if match:
            ...
            # Only return if it looks like a JSON object with empty values
