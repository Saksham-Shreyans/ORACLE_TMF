"""
ORACLE-TMF  ·  tests/stage2/test_nav_engine.py
================================================
Unit tests for the NAV (Negative Artifact Vector) engine.
Zero external dependencies — uses only stdlib + project models.

Coverage:
  • NAVEngine.run() with no previous version → skips gracefully
  • NAVEngine.run() detects drop in dead_code count
  • NAVEngine.run() detects multiple simultaneous drops
  • NAVEngine._compute_aggregate_score() normalisation
  • NAVEngine._compute_confidence_adjustment() sign/range
  • NAVEngine.update_history() builds history correctly
  • NAV-MIRAGE velocity computation
  • NAVResult.to_dict() round-trip
"""
import sys
import unittest
...
# ─── Minimal MAG stub ─────────────────────────────────────────────────────────
# We use plain objects to avoid pulling in the full androguard stack.
...
class FakeMAG:
    """Minimal MAG substitute for testing NAV without androguard."""
...
class TestNAVEngineNoHistory(unittest.TestCase):
    """NAV engine with no prior version — must skip gracefully."""
...
class TestNAVEngineDropDetection(unittest.TestCase):
    """NAV engine correctly detects artifact count drops."""

    def setUp(self):
        self.engine = NAVEngine()

    def test_dead_code_drop_detected(self):
        """8 dead code in v_prev, 2 in v_curr → NAV event."""
    ...
    def test_permission_drop_detected(self):
        """6 permissions in v_prev, 0 in v_curr → PERMISSION_REMOVAL."""
    ...
    def test_no_drop_no_event(self):
        """Same counts → no NAV events."""
        mag_prev = FakeMAG(dead_code_count=3, version="v1")
        mag_curr = FakeMAG(dead_code_count=5, version="v2")  # increase, not drop
        result = self.engine.run(mag_curr, mag_prev=mag_prev)
    ...
    def test_small_drop_below_threshold_ignored(self):
        """Drop of 1 (below NAV_MIN_DROP_COUNT=2) → no event."""
        mag_prev = FakeMAG(dead_code_count=3, version="v1")
        mag_curr = FakeMAG(dead_code_count=2, version="v2")  # drop of 1
        result = self.engine.run(mag_curr, mag_prev=mag_prev)
        dead_events = [e for e in result.nav_events if e.artifact_class == ArtifactClass.DEAD_CODE.value]
        self.assertEqual(len(dead_events), 0)

    def test_multiple_drops_detected(self):
        """Drops in multiple classes all detected."""
        mag_prev = FakeMAG(
...
class TestNAVScoreComputation(unittest.TestCase):
    """NAV score and confidence adjustment are correctly computed."""

    def setUp(self):
        self.engine = NAVEngine()

    def test_nav_score_in_unit_interval(self):
        """nav_score must be in [0.0, 1.0]."""
    ...
    def test_full_drop_scores_one(self):
        """Complete elimination of all artifacts → nav_score = 1.0."""
        mag_prev = FakeMAG(dead_code_count=10, version="v1")
        mag_curr = FakeMAG(dead_code_count=0, version="v2")
        result = self.engine.run(mag_curr, mag_prev=mag_prev)
...
class TestNAVRedirection(unittest.TestCase):
    """NAV redirection hypothesis is correctly assigned."""
    ...
    def test_primary_redirection_is_set_on_high_score_event(self):
        ...
        mag_curr = FakeMAG(dead_code_count=0, version="v2")
        result = self.engine.run(mag_curr, mag_prev=mag_prev)
        # primary_redirection should be set because there's a genuine NAV event
        # (DEAD_CODE → CAPABILITY_DELAYED by default)
        # Even if UNKNOWN, has_redirection is evaluated
        self.assertIsInstance(result.has_redirection, bool)


class TestNAVHistoryUpdate(unittest.TestCase):
    """NAV history is correctly built across multiple versions."""
    ...
    def test_first_appearance_tracked(self):
        history_map = {}
        # v1 has no dead code, v2 has some
...
class TestNAVResultSerialization(unittest.TestCase):
    """NAVResult.to_dict() produces valid, complete structure."""
