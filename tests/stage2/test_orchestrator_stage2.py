"""
ORACLE-TMF  ·  tests/stage2/test_orchestrator_stage2.py
=========================================================
Integration tests for the Stage 2 Orchestrator.
Tests the full Stage 2 pipeline coordination without requiring
LLM API keys, androguard, or Frida.

Coverage:
  Stage2Config:
    • Default config has correct defaults
    • Custom config overrides are respected

  Stage2Orchestrator:
    • Initialises without errors
    • run() with minimal MAG returns a Stage2Report
    • run() with NAV disabled skips Stage N
    • run() with single APK skips Stage O (CABAL)
    • Errors in individual stages do not crash the orchestrator
    • to_dict() produces a valid JSON-serialisable structure
    • Stage N adjusts forecast confidence when nav_adjustment is nonzero
    • Network Attack result is populated when enabled

  Stage2Report:
    • to_dict() structure contains all required top-level keys
...
class TestStage2Config(unittest.TestCase):
    """Stage2Config default and custom values."""
...
# ═══════════════════════════════════════════════════════════════════════════════
# Stage2Orchestrator tests
# ═══════════════════════════════════════════════════════════════════════════════

class TestStage2OrchestratorInit(unittest.TestCase):
    """Orchestrator initialises cleanly."""
...
class TestStage2OrchestratorRun(unittest.TestCase):
    """Stage2Orchestrator.run() produces valid Stage2Report."""

    def setUp(self):
        self.cfg = Stage2Config(
            phantom_enabled=False,    # No real device needed
            nav_enabled=True,
            cabal_enabled=False,
            ...
            save_json_reports=False,  # Don't write to disk during tests
        )
    ...
    def test_stage_m_skipped_when_disabled(self):
        report = self.orch.run(self.mag, forecasts=self.forecasts)
        # PHANTOM disabled → stage_m should be None or skipped
        if report.stage_m:
            self.assertTrue(report.stage_m.skipped)

    def test_stage_n_runs_without_prev(self):
        """Stage N with no mag_prev should skip gracefully."""
    ...
    def test_stage_n_runs_with_prev(self):
        """Stage N with mag_prev should run and produce a NAVResult."""
        mag_prev = FakeMAG(n_dead=10, n_perms=5)  # More artifacts in prev
    ...
    def test_stage_o_skipped_for_single_apk(self):
        """CABAL requires ≥2 APKs; single-APK run should skip it."""
        cfg = Stage2Config(cabal_enabled=True, save_json_reports=False)
        orch = Stage2Orchestrator(cfg)
        # No mag_list provided → Stage O should skip
    ...
    def test_network_attack_result_populated(self):
        """NetworkAttackResult should be present when enabled."""
        report = self.orch.run(self.mag, forecasts=self.forecasts)
    ...
    def test_error_in_one_stage_does_not_crash_orchestrator(self):
        """Exceptions in individual stages are caught — report is always returned."""
        # Use a deliberately bad config that may cause an internal issue
        cfg = Stage2Config(
            kinship_enabled=True,
            mirage_enabled=True,
        ...
        orch = Stage2Orchestrator(cfg)
        # This should always return a report, never raise
        try:
...
# ═══════════════════════════════════════════════════════════════════════════════
# Stage2Report.to_dict() tests
# ═══════════════════════════════════════════════════════════════════════════════

class TestStage2ReportDict(unittest.TestCase):
    """Stage2Report.to_dict() produces a complete, JSON-serialisable structure."""
...
class TestNAVForecastAdjustment(unittest.TestCase):
    """Stage N actually adjusts forecast confidence scores."""

    def test_nav_drop_adjusts_confidence(self):
        """A large NAV event (many artifacts dropped) should change forecast scores."""
        cfg = Stage2Config(
            nav_enabled=True,
            kinship_enabled=False,
        ...
        mag_prev = FakeMAG(n_dead=10, n_perms=5)  # Much more in prev
        ...
        # Adjusted forecasts may differ from originals when NAV adjustment ≠ 0
        # We just verify the adjusted_forecasts list exists and has the right count
        self.assertEqual(len(report.adjusted_forecasts), 1)
        # The adjusted score should be within a reasonable range
