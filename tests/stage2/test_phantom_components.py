"""
ORACLE-TMF  ·  tests/stage2/test_phantom_components.py
========================================================
Unit tests for PHANTOM sub-modules.
Zero external dependencies — no Frida, no API key required.

Coverage
--------
  SensoryEmulator:
    • OU process values within plausible physical ranges
    • Gyroscope variance > 0 (not a flat emulator value)
    • Light sensor values ≥ 0 (non-negative lux)
    • generate_stream produces correct count
    • sample_as_android_json schema is complete

  DevicePersonaGenerator:
    • Luhn check digit is valid for generated IMEI
    • Luhn check digit is valid for generated ICCID
    • IMSI length is 15 digits
    • Phone number starts with +91 prefix
    • Fingerprint contains expected persona fields
    • session_id is 16-character hex string
    • to_frida_context() contains all required keys
    • to_llm_context() is a non-empty string

  BehavioralBiometricGenerator:
    • simulate_typing() IKI values all in [30, 2000] ms range
    • Total duration > 0 for non-empty text
    • simulate_tap() returns exactly 2 TouchEvents (DOWN, UP)
    • simulate_swipe() has first action=DOWN, last action=UP
    • field_navigation_delay_ms() in expected range
"""
import math
import sys
import unittest
...
class TestSensoryEmulator(unittest.TestCase):
    """Tests for Ornstein-Uhlenbeck sensory emulation."""
    ...
    def test_gyroscope_within_physical_range(self):
        """Gyroscope should be within ±5 rad/s for a held device."""
    ...
    def test_gyroscope_has_nonzero_variance(self):
        """Variance must be > 0 — flat sensor = emulator detection by ToxicPanda."""
    ...
    def test_accelerometer_near_gravity(self):
        """Z-axis accelerometer should hover near 9.81 m/s² (gravity)."""
        samples = list(self.emulator.generate_stream(200))
    ...
    def test_android_json_schema_complete(self):
        """sample_as_android_json() must have all four sensor keys."""
        ...
        # Each entry must have timestamp and values
        for key in required_keys:
    ...
    def test_reset_restores_equilibrium(self):
        """After reset(), the emulator behaves as fresh (seed-independent test)."""
        _ = list(self.emulator.generate_stream(50))  # Warm up
        self.emulator.reset()
        # Post-reset, tick count should restart
        sample = self.emulator.next_sample()
        # Timestamp should be within a few milliseconds of time.time()
        import time
        t_now_ns = time.time_ns()
        self.assertLess(abs(sample.timestamp_ns - t_now_ns), 1_000_000_000)  # within 1 second


class TestDevicePersonaGenerator(unittest.TestCase):
    """Tests for AndroidPersona generation validity."""
    ...
    def test_imei_luhn_valid(self):
        """IMEI must pass Luhn checksum — malware validates this."""
        for _ in range(10):
            persona = self.gen.generate()
            self.assertTrue(
    ...
    def test_session_id_is_16_hex_chars(self):
        ...
        int(persona.session_id, 16)  # raises ValueError if not hex
    ...
    def test_two_sessions_have_different_imei(self):
        """Each session generates a unique IMEI."""
...
class TestBehavioralBiometricGenerator(unittest.TestCase):
    """Tests for synthetic keystroke and touch event generation."""
    ...
    def test_iki_values_in_valid_range(self):
        """All IKI values must be within [30, 2000] ms."""
        session = self.gen.simulate_typing("Hello World")
        for event in session.key_events[1:]:  # Skip first (IKI = 0 by convention)
    ...
    def test_tap_coordinates_close_to_target(self):
        """Touch coordinates should land within ±60px of target."""
    ...
    def test_cognitive_load_increases_timing(self):
        """Higher cognitive_load → longer mean IKI."""
