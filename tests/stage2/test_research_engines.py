"""
ORACLE-TMF  ·  tests/stage2/test_research_engines.py
======================================================
Unit tests for Stage 2 research engines.
Zero external dependencies (no LLM API key, no sentence-transformers required).

Coverage:
  KINSHIP BDV extraction:
    • char n-gram frequencies are normalised
    • opcode n-gram frequencies are normalised
    • entropy statistics are correct
    • cosine similarity of identical BDVs = 1.0
    • cosine similarity of zero vectors = 0.0
    • to_text_representation() is non-empty

  MIRAGE robustness analysis:
    • robustness_score in [0.0, 1.0]
    • most_vulnerable_class is one of the known artifact classes
    • candidates have correct hardness assignments
    • cost_score in [0.0, 1.0] for every candidate
    • recommendations are non-empty strings

  CABAL edge detection:
    • same-package links are excluded (no self-collusion)
    • pattern matching scores correct for known patterns
    • path finding produces CollusionPath objects with correct hop counts
    • to_dict() structure is valid

  Network Attack Analyzer:
    • analyze() returns a valid NetworkAttackResult
    • to_dict() structure is valid
    • DGA detection identifies java.util.Random + Date patterns
    • Empty MAG → no threats detected
"""
import sys
import unittest
...
class TestKINSHIPBDVExtraction(unittest.TestCase):
    """KINSHIP BuilderDNAVector extraction from a MAG."""
    ...
    def test_char_ngram_frequencies_normalised(self):
        ...
        if bdv.char_ngram_freq:
            ...
            # Normalised frequencies should sum to ~1.0 (or be individually < 1)
...
class TestMIRAGERobustness(unittest.TestCase):
    """MIRAGE adversarial robustness engine produces correct scores."""
    ...
    def test_easy_class_has_lower_cost_than_hard(self):
        """Unused permissions (easy) must have lower cost than dead_code_scaffolding (hard)."""
        result = self.engine.analyze(self._make_mag())
    ...
    def test_hardness_assignments_correct(self):
        """Each candidate's hardness must match the config specification."""
        from config.stage2_settings import MIRAGE_INJECTION_COSTS
        result = self.engine.analyze(self._make_mag())
        for candidate in result.poisoning_candidates:
...
class TestCABALEngine(unittest.TestCase):
    """CABAL cross-app collusion engine basic functionality."""
    ...
    def test_single_apk_produces_empty_result(self):
        """CABAL requires ≥2 APKs — single APK should return no paths."""
    ...
    def test_two_apks_no_overlap_no_edges(self):
        """Two APKs with unrelated intents → no collusion edges."""
    ...
    def test_same_package_not_colluding(self):
        """Stubs and filters from the same package are never a collusion edge."""
        ...
        # Same package: score doesn't matter because same-package pairs are excluded in run()
        # But we test compute_compatibility still returns a value
        self.assertGreaterEqual(score, 0.0)

    def test_pattern_matching_sms_bridge(self):
        """SMS stub + SMS filter → compatibility score ≥ 0.75."""
        from research.cabal.collusion_engine import IntentStub, IntentFilter
        stub = IntentStub(
    ...
    def test_direct_action_match_scores_one(self):
        """Exact action match → pattern score = 1.0."""
        from research.cabal.collusion_engine import IntentStub, IntentFilter
    ...
    def test_mitre_mapping_returns_string(self):
        ...
        self.assertIn("T", result_str)  # All MITRE IDs start with T
...
from research.cabal.collusion_engine import CABALEngine  # for _map_to_mitre static method


# ═══════════════════════════════════════════════════════════════════════════════
# Network Attack Analyzer Tests
# ═══════════════════════════════════════════════════════════════════════════════

class TestNetworkAttackAnalyzer(unittest.TestCase):
    """Network Attack Analyzer correctly classifies DDoS signatures."""
    ...
    def test_syn_flood_signature_detected(self):
        """Dead code containing raw socket signature → SYN Flood detected."""
    ...
    def test_dga_pattern_detected(self):
        """Smali with Random + Date + TLD patterns → DGA detected."""
        dga_smali = (
    ...
    def test_dga_seed_source_timestamp(self):
        """getTime() + Random → seed_source = TIMESTAMP."""
        dga_smali = (
