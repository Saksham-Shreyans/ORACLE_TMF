...
class TestDTEEngineInit(unittest.TestCase):
    ...
    def test_model_n_estimators(self):
        engine = DTEEngine()
        # XGBoost stores n_estimators as attribute
...
class TestDTEClassification(unittest.TestCase):
    ...
    def test_labels_assigned(self):
        ...
        # Result may be shorter than input (REMNANTs filtered out)
        for a in result:
    ...
    def test_remnant_profile_filtered(self):
        ...
        """
        # REMNANT profile: trigger=0, entropy=0.5, api=0.05, indegree=10
        ...
        # Either filtered out (REMNANT) or retained with non-REMNANT label
        if result:
            self.assertNotEqual(result[0].dte_label, DTEClass.REMNANT)
        # No assertion if empty — filtering worked
    ...
    def test_original_list_labels_updated(self):
        ...
        # Input artifact should now have a non-default label
...
class TestDTESyntheticData(unittest.TestCase):
    ...
    def test_synthetic_data_shapes(self):
        ...
        # ~8700 samples total (5000+3000+500+200)
