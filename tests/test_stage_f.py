...
class TestShannonEntropy(unittest.TestCase):
    ...
    def test_uniform_string_max_entropy(self):
        ...
        s = "abcdefgh"   # 8 unique characters
    ...
    def test_known_high_entropy_string(self):
        ...
        # A random-ish string with mixed case + digits
    ...
    def test_two_char_equal_frequency(self):
        h = StringMiner._shannon_entropy("ababababab")   # p(a)=0.5, p(b)=0.5
...
class TestPlaceholderPatterns(unittest.TestCase):
    ...
    def test_hardcoded_ipv4_no_fire_on_localhost(self):
        # "localhost" and short non-IP strings should not match
...
class TestStringEvaluation(unittest.TestCase):
    ...
    def test_high_entropy_string_flagged(self):
        # Entropy > STRING_HIGH_ENTROPY_THRESHOLD (4.5)
    ...
    def test_staging_url_flagged(self):
        # Use a URL with "local" in it — triggers staging_url pattern (localhost match)
        # but does NOT contain dev/test/staging/debug/sandbox keywords
        ...
        # matched_pattern will be whichever pattern fires first (staging_url or test_marker)
    ...
    def test_benign_prefix_not_flagged_by_entropy(self):
        ...
        """
        # This test validates the benign prefix filter in _mine_dex_string_pool.
        # The _evaluate_string method itself doesn't filter prefixes —
        # that filter is applied at the mining layer.
        # We just verify _evaluate_string CAN return None for low-risk strings.
        s = "com.google.firebase.FirebaseApp"
        artifact = self.miner._evaluate_string(s, source="string_pool")
        # This may or may not be flagged by entropy — just ensure no crash
        # The prefix filter happens BEFORE _evaluate_string is called in mining
